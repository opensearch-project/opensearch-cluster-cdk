...
| feature                       | Optional    | string  | Enables bundled tuning for specific features. Currently only `datafusion` is supported, which forces data nodes (including the seed data node) to use an 8GB heap and injects default `additionalConfig` values (`indexing_pressure.memory.limit` and `datafusion.search.memory_pool`). User-supplied `additionalConfig` entries override these defaults. |
| pluginUrl                     | Optional    | string  | URL to a custom plugin zip that should be installed on every node (via `bin/opensearch-plugin`). When `feature=datafusion` is supplied and `pluginUrl` is not set, the stack automatically injects the DataFusion engine plugin URL for the selected `distVersion`. |
