...
| feature                       | Optional    | string  | Enables bundled tuning for specific features. Currently only `datafusion` is supported, which forces data nodes to use an 8GB heap and injects default `additionalConfig` values (`indexing_pressure.memory.limit` and `datafusion.search.memory_pool`). User-supplied `additionalConfig` entries override these defaults. |
