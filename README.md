...
| managerInstanceType           | Optional    | string  | EC2 instance type for cluster manager nodes. Defaults to c5.xlarge for x64 and c6g.xlarge for arm64. Supports the same instance types as `dataInstanceType`. E.g., `-c managerInstanceType=m5.xlarge`                                                                                                                                                                                                                                                                                                                                                                        |
...
| storageVolumeType             | Optional    | string  | EBS volume type for all the nodes (data, ml, cluster manager). Defaults to gp3. See `lib/opensearch-config/node-config.ts` for available options. E.g., `-c storageVolumeType=gp3`. For SSD based instance (i.e. i3 family), it is used for root volume configuration.                                                                                                                                                                                                                                                                                                                                                                       |
...
| enableGrpc                    | Optional    | boolean | Enable gRPC endpoints via the `transport-grpc` module. Requires OpenSearch 3.0+. e.g., `--context enableGrpc=true`. Defaults to false.                                                                                                                                                                                                           |
| mapGrpcPortTo                 | Optional    | integer | Load balancer port number to map to gRPC endpoint. e.g., `--context mapGrpcPortTo=9450`. Defaults to 9450 when gRPC is enabled.                                                                                                                                                                                                                                                                    |
...
To interact with the gRPC endpoint (when `enableGrpc=true`), use the configured gRPC port (default `9450`):
```
grpcurl -plaintext <load-balancer-url>:9450 list
```
